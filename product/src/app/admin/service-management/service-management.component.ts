...
export class ServiceManagementComponent implements OnInit {
  ...
  loadServices(): void {
    this.isLoading = true;
    this.bookingService.getServices().subscribe((data: SpaServiceData[]) => {
      this.services = data;
  ...
  onFormSubmit(form: NgForm): void {
    ...
    if (this.isEditing) {
      this.bookingService.updateService(this.serviceModel.id, this.serviceModel).subscribe({
        next: (updatedService: SpaServiceData) => {
        ...
        error: (err: any) => this.errorMessage = 'Failed to update service.'
      });

    } else {
      this.bookingService.createService(this.serviceModel).subscribe({
        next: (newService: SpaServiceData) => {
          this.services.push(newService);
          this.resetForm(form);
        },
        error: (err: any) => this.errorMessage = 'Failed to create service.'
      });
    }
  }
  ...
  onDelete(service: SpaServiceData): void {
    ...
    this.bookingService.deleteService(service.id).subscribe({
      ...
      error: (err: any) => this.errorMessage = 'Failed to delete service. It may be linked to existing bookings.'
    });
  }
